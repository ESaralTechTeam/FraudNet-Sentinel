...
async def create_complaint_record(
    ...
    try:
        ...
        # Generate fallback beneficiary_id if missing or "UNKNOWN"
        if not beneficiary_id or beneficiary_id == "UNKNOWN":
            beneficiary_id = f"BEN-{random.randint(100000, 999999)}"
            print(f"Generated fallback beneficiary_id: {beneficiary_id}")
        ...
        # Log the stored item for verification
        print(f"✅ Complaint created: {complaint_id} with beneficiary_id: {beneficiary_id}")
        ...
        print(f" Complaint creation error: {str(e)}")
